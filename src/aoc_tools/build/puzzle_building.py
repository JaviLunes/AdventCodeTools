...
class AdventBuilder:
    ...
    def build_templates(self, day: int):
        ...
        for file_path, template_file in self.paths.build_templates_map(day=day).items():
            file_lines = self._prepare_file_lines(template_file=template_file, day=day)
    ...
    def _prepare_file_lines(self, template_file: Path, day: int) -> list[str]:
        ...
        for mark, value in self._get_replace_map(day=day).items():
            lines_str = lines_str.replace(mark, value)
        return lines_str.split("|")

    def _get_replace_map(self, day: int) -> dict[str, str]:
